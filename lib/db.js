...
class DB {
  ...
  constructor (_location, id, db, options) {
    ...
    this._levelup = new Levelup(leveldown, Object.assign({}, options));
  ...
  persist (state, done) {
    ...
    this._getPersistBatch(state, (err, batch) => {
      if (err) {
        ...
        //console.log('persisting state', batch)
  ...
  command (state, command, options, done) {
    console.log('command0', state, command, options)
    this._getPersistBatch(state, (err, batch) => {
      if (err) {
        done(err)
      } else {
        ...
        console.log('%s: going to apply batch: %j', this.id, batch)
        this.db.batch(batch, err => {
          console.log('%s: applied batch command err = %j', this.id, err)
          if (!err) {
            if (isQuery) {
