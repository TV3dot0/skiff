...
class DB {
  ...
  persist (state, done) {
    ...
    this._getPersistBatch(state, (err, batch) => {
      if (err) {
        ...
        //batch.map(o => debug(o.key, ':', o.value))
  ...
  command (state, command, options, done) {
    this._getPersistBatch(state, (err, batch) => {
      if (err) {
        ...
        //debug('command0', this.id, command.type, command.key?command.key.toString('hex'):'');
        if (typeof command.type === 'undefined') debug('undefined', command);
        ...
        this.db.batch(batch, err => {
          ...
          if (!err) {
            if (isQuery) {
              //debug('command query', command.key.toString('hex'))
  ...
  applyEntries (entries, applyTopology, done) {
    if (entries.length) {
      //debug('applying entries', this.id,
  ...
  _getPersistLog (state, _done) {
    //debug('%s: persisting log', this.id, state.logEntries());
    ...
    this.log.createKeyStream()
      .on('data', key => {
        if (!byKey.hasOwnProperty(key)) {
          ...
          debug('remove key', this.id, key);
          removeKeys.push(key)
        } else {
          // remove entries already in the database
          debug('delete entry by key', this.id, key);
          delete byKey[key]
        }
      })
      .once('error', done)
