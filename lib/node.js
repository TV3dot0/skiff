...
class Node extends EventEmitter {
  ...
  _handleRequest (message, done) {
    ...
    if (from) {
      ...
      if (this._state) {
        ...
        console.log('error: no state available to handle request');
    ...
    if (this._stateName !== 'leader') {
      ...
      if (!options.remote) {
        ...
        if (Array.isArray(command)) {
          //console.log('state command batch @follower', this.stateId, command);
          await command
          ...
          //console.log('state command @follower', this.stateId, command.type, command.key ? command.key.toString('hex') : '');
      ...
      if (Array.isArray(command)) {
        //console.log('state command @leader', this.stateId, command);
        await command
        ...
        //console.log('state command @leader', this.stateId, command.type, command.key ? command.key.toString('hex') : '');
  ...
  _applyTopologyCommands (commands) {
    //console.log('%s: _applyTopologyCommands %j', this.id, commands)
    commands.forEach(this._applyTopologyCommand.bind(this))
  }

  _applyTopologyCommand (command) {
    //console.log('%s: applying topology command: %j', this.id, command)
    if (command.type === 'join') {
      ...
      //console.log('%s: applying leave command: %j', this.id, command)
  ...
  _segregatePeer (peer) {
    //console.log('%s: segregating peer', this.id, peer)
    this._peers = this._peers.filter(p => p !== peer)
    //console.log('%s: peers now are: %j', this.id, this._peers)
    this._state.leave(peer)
    if (this._network) {
      this._network.active.disconnect(peer)
    }
    this.emit('left', peer)
    //console.log('%s: emitted left for peer', this.id, peer)
  }
