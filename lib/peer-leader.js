...
class PeerLeader extends EventEmitter {
    ...
    if (entries) {
      ...
      this._node.network.rpc(
        ...
        (err, reply) => { // callback
          ...
          if (err) {
            console.log('%s: error on AppendEntries reply:\n%s', this._node.state.id, err.stack)
      ...
      console.log('%s: peer %s is lagging behind (next index is %d), going to install snapshot',
  ...
  _installSnapshot () {
    console.log('%s: _installSnapshot on %s', this._node.state.id, this._address)
    ...
    function installSnapshot (data) {
      console.log('%s: have chunks %j, finished = %j', self._node.state.id, data.chunks, data.finished)
      console.log('%s: installSnapshot on leader: have chunks %j, finished = %j', self._node.state.id, data.chunks, data.finished)
      stream.pause()
      ...
      self._node.network.rpc(
        ...
        (err, reply) => { // callback
          console.log('%s: got InstallSnapshot reply', self._node.state.id, err, reply)
          if (err) {
            cleanup()
          } else {
            if (data.finished) {
      ...
      console.log('%s: sent InstallSnapshot', self._node.state.id)
    }
