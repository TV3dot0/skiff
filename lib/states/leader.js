...
class Leader extends Base {
  ...
  constructor (node, _options) {
    ...
    this.name = 'leader';
    this._followers = {}
    console.log('new leader', node.id)
  }

  start () {
    console.log('%s is leader', this.id);
    console.log('peers', this._node.network.peers());
    ...
    console.log('leader followers', Object.keys(this._followers));
    super.start();
